> **직전 영어본 보존 사본 (대조용 증거, 런타임 아님)**
>
> 2026-08-25 한글화 재빌드 직전의 `.claude/skills/om-verify/SKILL.md`(225줄) 본문이다.
> 1차 검토(`검토결과_om-verify_1차_20260825.md`)가 **승인**한 판본이며, 4차 독립 감사(`AUDIT_REPORT_ROUND4.md`)의 감사 대상이다.
>
> **출처 고지**: 이 저장소의 `.claude/skills/om-verify/`는 커밋된 적이 없어 Git에서 복원할 수 없다.
> 이 사본은 한글화 직전 저작 세션이 `cat -n`으로 읽은 전문을 그대로 옮긴 것이며, 바이트 동일성을 기계로 증명하지는 못한다.
> 의미 등가 대조의 기준 텍스트로만 사용하고, 런타임 스킬로 취급하지 않는다.
